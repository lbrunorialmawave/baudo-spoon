...
from ml.optimizer.diversity import (
    NearOptimalConfig,
...
def _enrich_pool_with_blends(
    pool: list[Player], config: OptimizationConfig, req: OptimizationRequest
) -> list[Player]:
    ...
    if config.hybrid_blend > 0 and config.ruleset != "MANTRA":
        ...
        if report.scores:
            pool = [
                replace(p, fp_ibrido=report.scores[p.player_id])
                if p.player_id in report.scores
                else p
                for p in pool
            ]
        else:
            log.warning(
                "hybrid_blend requested but no fp_ibrido data found: %s",
                report.warnings,
            )
...
def _mc_config_from_request(
    ...
    *,
    ...
    if mc is None:
        ...
        risk_lambda, min_freq, seed = (
            mc.risk_lambda,
            mc.min_selection_frequency,
            mc.random_seed,
        )
        timeout = mc.timeout_seconds or settings.optimizer_saa_timeout_seconds
    if mode not in ("mean_std", "saa_frequency"):
        raise HTTPException(
            status_code=422, detail=f"invalid monte_carlo.mode {mode!r}"
        )
    ...
    return MonteCarloOptConfig(
        ...
        min_selection_frequency=min_freq,
        random_seed=seed,
...
def _near_cfg_from_request(req: OptimizationRequest) -> NearOptimalConfig | None:
    ...
    if n is None or not n.enabled:
        return None
    return NearOptimalConfig(
        enabled=True,
        n_alternatives=n.n_alternatives,
        exclude_top_m=n.exclude_top_m,
        max_score_drop_pct=n.max_score_drop_pct,
    )
    ...
    for alt in alts:
        ...
        out.append(
            NearOptimalAlternativeSchema(
                ...
                score_delta=round(alt.score_delta, 4),
                score_delta_pct=round(alt.score_delta_pct, 4),
                squad=squad,
                total_projected_score=alt.result.total_projected_score,
                status=alt.result.status,
            )
        )
    return out
...
def _with_pool_diagnostics(result, pool, *, extra=None):
    ...
    return dc_replace(
        result,
        diagnostics=merge_result_diagnostics(
            result, build_pool_diagnostics(pool), extra=extra
        ),
    )
...
def _custom_strategies(schemas: list[StrategyProfileSchema]) -> list[StrategyProfile]:
    ...
    for s in schemas:
        ...
        try:
            ...
            raise HTTPException(
                status_code=422, detail=f"Invalid custom strategy {s.name!r}: {exc}"
            ) from exc
    return out
...
def _serialize_result(
    ...
    near_optimal: list[NearOptimalAlternativeSchema] | None = None,
) -> OptimizationResultSchema:
...
def _serialize_strategy(s: StrategyProfile) -> StrategyProfileSchema:
    ...
    if s.min_budget_share_by_roles is None:
        mbsr: tuple[list[str], float] | None = None
    else:
...
async def run_multi_strategy(
    ...
    try:
        if mc_cfg is not None:
            ...
            for strat in strategies:
                saa = await asyncio.to_thread(
                    run_monte_carlo_opt, pool, config, strat, mc_cfg, sim
                )
                if saa.representative is None:
                    continue
                multi_results[strat.name] = _with_pool_diagnostics(
                    saa.representative,
                    pool,
                    extra={
            ...
            if not multi_results:
                raise HTTPException(
                    status_code=400, detail="Monte Carlo produced no squads"
                )
        else:
            ...
            multi_results = {
                n: _with_pool_diagnostics(r, pool) for n, r in multi.results.items()
            }
    ...
    if getattr(req, "diversify_strategies", False) and len(multi_results) > 1:
        ...
        multi_results = {
            n: _with_pool_diagnostics(r, pool) for n, r in multi_results.items()
        }
    ...
    for name, res in multi_results.items():
        ...
        if near_cfg is not None and name == first_name:
            ...
            alts = await asyncio.to_thread(
                generate_near_optimal_alternatives, pool, config, strat, res, near_cfg
            )
...
async def run_single_strategy(
    ...
    if req.custom_strategies:
        if len(req.custom_strategies) != 1:
            raise HTTPException(
                status_code=422,
                detail="custom_strategies must have exactly 1 entry for /single",
            )
        ...
        raise HTTPException(
            status_code=422, detail="Provide strategy_name or custom_strategies"
        )
    repo = DataRepository(
        artifacts_dir=settings.artifacts_dir,
        r2_endpoint_url=settings.r2_endpoint_url,
    ...
    try:
        if mc_cfg is not None:
            saa = await asyncio.to_thread(
                run_monte_carlo_opt, pool, config, strategy, mc_cfg
            )
            if saa.representative is None:
                raise HTTPException(
                    status_code=400, detail="Monte Carlo produced no squad"
                )
            result = _with_pool_diagnostics(
                saa.representative,
                pool,
                extra={
                    "mc_wall_time_seconds": saa.wall_time_seconds,
            ...
            result = _with_pool_diagnostics(
                await asyncio.to_thread(optimize_squad, pool, config, strategy), pool
            )
    except PreFlightError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    ...
    if near_cfg is not None:
        alts = await asyncio.to_thread(
            generate_near_optimal_alternatives, pool, config, strategy, result, near_cfg
        )
        near_schemas = _serialize_near_optimal(alts, effective_lookup)
    wp = (
    ...
    return _serialize_result(
        ...
        effective_lookup,
        win_probability=wp,
        monte_carlo_summary=mc_summary_schema,
        near_optimal=near_schemas,
    )
...
async def run_sensitivity(
    ...
    if req.custom_strategies:
        if len(req.custom_strategies) != 1:
            raise HTTPException(
                status_code=422,
                detail="custom_strategies must have exactly 1 entry for /sensitivity",
            )
        strategy = _custom_strategies(req.custom_strategies)[0]
    elif strategy_name:
        strategy = _strategy_by_name(strategy_name)
    ...
    if not pool:
        raise HTTPException(
            status_code=400,
            detail=f"Empty player pool for season_start={req.season_start}",
        )
    ...
    enrich_cfg = dc_replace_cfg(
        config,
        var_blend=max(config.var_blend, 0.5),
        hybrid_blend=max(config.hybrid_blend, 0.5),
    )
    pool = _enrich_pool_with_blends(pool, enrich_cfg, req)

    try:
        result = await asyncio.to_thread(
            compute_sensitivity_matrix, pool, config, strategy
        )
...
async def run_pareto(
    ...
    if req.custom_strategies:
        if len(req.custom_strategies) != 1:
            raise HTTPException(
                status_code=422,
                detail="custom_strategies must have exactly 1 entry for /pareto",
            )
        strategy = _custom_strategies(req.custom_strategies)[0]
    elif strategy_name:
        strategy = _strategy_by_name(strategy_name)
    ...
    try:
        result = await asyncio.to_thread(
            compute_pareto_frontier, pool, config, strategy
        )
    except PreFlightError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
def _run_mc_job_sync(job_id, pool, config, strategy, mc_cfg):
    try:
        ...
        if saa.representative is None:
            job_store.set_failed(job_id, "Monte Carlo produced no representative squad")
            return
        result = _with_pool_diagnostics(
            saa.representative,
            pool,
        ...
        schema = _serialize_result(
            result, effective_lookup, monte_carlo_summary=_serialize_mc_summary(summary)
        )
        job_store.set_completed(
            job_id, result=schema.model_dump(), monte_carlo_summary=summary
        )
...
async def create_optimize_job(
    req: OptimizationRequest, strategy_name: str = "Bilanciata", db=Depends(get_db)
):
    ...
    if mc_cfg is None:
        raise HTTPException(
            status_code=422, detail="monte_carlo.enabled must be true for async jobs"
        )
    strategy = (
        _strategy_by_name(strategy_name)
        if not req.custom_strategies
        else _custom_strategies(req.custom_strategies)[0]
    )
    config = _build_config(req)
    if req.pool_override is not None:
        pool = _pool_from_override(req)
    ...
    asyncio.get_running_loop().run_in_executor(
        None, _run_mc_job_sync, job.job_id, pool, config, strategy, mc_cfg
    )
...
async def get_optimize_job(job_id: str):
    ...
    result_schema = (
        OptimizationResultSchema(**job.result) if job.result is not None else None
    )
    mc_summary = (
        MonteCarloSummarySchema(**job.monte_carlo_summary)
        if job.monte_carlo_summary is not None
        else None
    )
