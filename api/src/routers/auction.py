...
def _bidder_profile_from_schema(p: BidderProfileSchema) -> BidderProfile:
    return BidderProfile(
        participant_id=p.participant_id, policy=_bidder_policy_from_schema(p.policy)
    )
...
def _sim_config_from_schema(cfg: object) -> AuctionSimulationConfig:
    ...
    return AuctionSimulationConfig(
        ...
        price_noise_std_ratio=c.price_noise_std_ratio,
...
async def simulate_auction_endpoint(
    ...
    if payload.player_pool is not None:
        ...
        if not pool:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Empty player pool"
            )
    ...
    if store_after != store_before:
        logger.error(
            "simulate_auction mutated auction_sessions (%d -> %d)",
            store_before,
            store_after,
        )
    ...
    return AuctionSimulationResponse(
        ...
        per_participant={
            pid: ParticipantSimStatsSchema(
                spend_p10=s.spend_p10,
                spend_p50=s.spend_p50,
                spend_p90=s.spend_p90,
                esv_total_p10=s.esv_total_p10,
                esv_total_p50=s.esv_total_p50,
                esv_total_p90=s.esv_total_p90,
        ...
        player_acquisition_probability={
            pid: PlayerAcquisitionStatsSchema(
                prob=st["prob"], avg_price=st["avg_price"]
            )
