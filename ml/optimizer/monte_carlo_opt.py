    ...
    def to_summary_dict(self) -> dict:
        return {
            ...
            "selection_frequency": {
                k: round(v, 4)
                for k, v in sorted(
                    self.selection_frequency.items(), key=lambda x: -x[1]
                )
            },
            "squad_score_percentiles": {
                k: round(v, 3) for k, v in self.squad_score_percentiles.items()
            },
...
def build_simulator_from_residuals(
    residuals: list[dict], *, random_seed: int = 42
) -> MonteCarloSimulator:
    return MonteCarloSimulator(random_seed=random_seed).fit(residuals)


def build_simulator_from_pool(
    pool: Sequence[Player], *, random_seed: int = 42, n_synthetic: int = 40
):
    warnings: list[str] = []
    residuals: list[dict] = []
    ...
    for p in pool:
        if p.prediction_std is not None and p.prediction_std > 0:
            ...
            for val in rng.normal(0.0, float(p.prediction_std), size=n_synthetic):
                residuals.append(
                    {"player_id": p.player_id, "role": p.role, "residual": float(val)}
                )
    if n_with_std == 0:
        warnings.append(
            f"No prediction_std; parametric fallback DEFAULT_STD={DEFAULT_STD}"
        )
...
def _simulate_pool(sim, pool, n_simulations):
    payload = [
        {"player_id": p.player_id, "predicted_score": p.projected_score, "role": p.role}
        for p in pool
    ]
    return {
        r.player_id: r for r in sim.simulate_many(payload, n_simulations=n_simulations)
    }
...
def run_mean_std(pool, config, strategy, mc_config, simulator=None):
    ...
    if simulator is None:
        simulator, warnings = build_simulator_from_pool(
            pool, random_seed=mc_config.random_seed
        )
    ...
    adjusted = [
        replace(
            ...
            projected_score=max(
                0.0,
                sim_map[p.player_id].mean_score
                - mc_config.risk_lambda * sim_map[p.player_id].std_score,
            ),
        )
        for p in pool
    ]
    ...
    return SAAResult(
        ...
        selection_frequency={p.player_id: 1.0 for p in result.squad}
        if result.squad
        else {},
        stability_index=1.0 if result.squad else 0.0,
        mean_pairwise_jaccard=1.0 if result.squad else 0.0,
        squad_score_percentiles={
            "p10": result.total_projected_score,
            "p50": result.total_projected_score,
            "p90": result.total_projected_score,
        },
...
def run_saa_frequency(pool, config, strategy, mc_config, simulator=None):
    ...
    for k in range(mc_config.n_simulations):
        if (
            mc_config.timeout_seconds > 0
            and (time.perf_counter() - t0) > mc_config.timeout_seconds
        ):
            warnings.append(f"SAA stopped early at {k}/{mc_config.n_simulations}")
            break
        scenario_pool = [
            replace(
                p,
                projected_score=max(
                    0.0, float(sim_map[p.player_id].simulated_scores[k])
                ),
            )
            for p in pool
        ]
        try:
            res = optimize_squad(
                scenario_pool, config, strategy, warm_start=prev_warm_start
            )
        except Exception as exc:
            warnings.append(f"scenario {k} failed: {exc}")
            continue
        completed += 1
        if res.squad:
            ...
            scenario_scores.append(
                sum(pool_by_id[pid].projected_score for pid in ids if pid in pool_by_id)
            )
    freq = (
        {pid: cnt / completed for pid, cnt in selection_counts.items()}
        if completed
        else {}
    )
    mean_jaccard = 0.0
    if len(scenario_squads) >= 2:
        ...
        for i in range(sn):
            for j in range(i + 1, sn):
                total_j += _jaccard(scenario_squads[i], scenario_squads[j])
                pairs += 1
        mean_jaccard = total_j / pairs if pairs else 0.0
    elif len(scenario_squads) == 1:
        mean_jaccard = 1.0
    mean_pool = [
        replace(p, projected_score=max(0.0, sim_map[p.player_id].mean_score))
        for p in pool
    ]
    try:
        representative = optimize_squad(mean_pool, config, strategy)
    except Exception as exc:
        warnings.append(f"representative solve failed: {exc}")
        representative = None
    stability = (
        float(np.mean([freq.get(sp.player_id, 0.0) for sp in representative.squad]))
        if representative and representative.squad and completed
        else 0.0
    )
    percentiles = {}
    if scenario_scores:
    ...
    if (
        mc_config.min_selection_frequency > 0.0
        and representative
        and representative.squad
        and completed
    ):
        fragile = sorted(
            (
    ...
    return SAAResult(
        ...
        squad_score_percentiles=percentiles,
