...
def _load_predictions_by_id(
    ...
    if not path.is_file():
        log.debug(
            "Predictions artefact not found at %s; season_value will be None.", path
        )
        return {}
    try:
...
def load_data(
    ...
    df["stats_from_prior_season"] = (
        df["stats_from_prior_season"].fillna(False).astype(bool)
    )
    df["stats_from_foreign_league"] = (
        df["stats_from_foreign_league"].fillna(False).astype(bool)
    )
    ...
    if df["stats_from_foreign_league"].any():
        ...
        if in_serie_b.any():
            for col in _SERIE_B_DISCOUNT_COLS:
                if col in df.columns:
                    df.loc[in_serie_b, col] = (
                        df.loc[in_serie_b, col] * _SERIE_B_DISCOUNT
                    )
            log.info(
...
def run_mantra(
    ...
    log.info(
        "Config: P1=%.2f P2=%.2f P3=%.2f P4=%.2f",
        cfg.PESO_P1,
        cfg.PESO_P2,
        cfg.PESO_P3,
        cfg.PESO_P4,
    ...
    team_df = (
        df[["team"] + [c for c in team_cols if c in df.columns]]
    ...
    scores = compute_fp_corr(
        fp, cp, df["ruolo_primario"], df["num_ruoli"], df["Pz1"], cfg
    )

    # 4. Fase 7
    log.info("Classifying Fase 7 …")
    fase7_label, fase7_motivo = classify_fase7(
        df, fp, scores["fp_mantra"], scores["vr"], p1, cfg
    )
    ...
    for idx in df.index:
        ...
        player_fotmob_id: int | None = int(raw_fotmob) if pd.notna(raw_fotmob) else None
        ...
        players_out.append(
            {
                ...
                "stats_from_foreign_league": bool(
                    df.at[idx, "stats_from_foreign_league"]
                ),
                # Computed pillars
                ...
                "Percentile_Ruolo": round(
                    float(scores["percentile_ruolo"].iloc[idx]), 4
                ),
                "Fase7": str(fase7_label.iloc[idx])
                if pd.notna(fase7_label.iloc[idx])
                else None,
                "Fase7_Motivo": str(fase7_motivo.iloc[idx])
                if pd.notna(fase7_motivo.iloc[idx])
                else None,
                "rischio": str(classification_8g.iloc[idx])
                if pd.notna(classification_8g.iloc[idx])
                else None,
