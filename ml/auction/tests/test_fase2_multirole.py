...
def _mk_player(
    pid: str, role: str, cost: int, score: float, eligible: frozenset[str] = frozenset()
) -> Player:
    return Player(
        player_id=pid,
        name=pid,
...
def test_select_scarcest_replacement_picks_highest_score() -> None:
    replacement_by_role = {
        ...
        "E": ReplacementLevel(
            role="E", score=6.5, n_players_used=1
        ),  # thinner pool -> scarcer
    }
...
def test_select_scarcest_replacement_ignores_roles_absent_from_pool() -> None:
    ...
    picked = _select_scarcest_replacement(
        frozenset({"Dd", "GhostRole"}), replacement_by_role
    )
...
def test_multi_role_player_var_uses_scarcest_eligible_role() -> None:
    ...
    )  # use full pool as "replacement" bucket for a crisp test
...
def test_single_eligible_role_matches_plain_role_baseline() -> None:
    ...
    engine = VarEngine()
    plain_result = next(
        r for r in engine.evaluate(plain_pool) if r.player_id == "target"
    )
...
def test_resolve_pricing_role_falls_back_when_no_eligible_role_in_index() -> None:
    ...
    cfg = AuctionConfig(num_participants=4)
    player = _mk_player(
        "p1", "D", 10, 5.0, eligible=frozenset({"NotInIndex1", "NotInIndex2"})
    )
