...
def _small_pool():
    players = []
    for i in range(6):
        players.append(_mk_player(f"p{i}", f"GK {i}", "P", 5 + i, 5.0 + i * 0.2))
    for i in range(10):
        players.append(_mk_player(f"d{i}", f"DF {i}", "D", 4 + i, 5.0 + i * 0.15))
    for i in range(10):
        players.append(_mk_player(f"c{i}", f"MF {i}", "C", 6 + i, 5.5 + i * 0.15))
    for i in range(8):
        players.append(_mk_player(f"a{i}", f"FW {i}", "A", 8 + i, 6.0 + i * 0.2))
    return players
...
def _two_participants(budget=300):
    return [
        ParticipantSetup(
            participant_id="u0", display_name="User 0", budget_initial=budget
        ),
        ParticipantSetup(
            participant_id="u1", display_name="User 1", budget_initial=budget
        ),
    ]
...
def test_n_simulations_out_of_range():
    with pytest.raises(ValueError, match="n_simulations"):
        AuctionSimulationConfig(n_simulations=0)
    with pytest.raises(ValueError, match="n_simulations"):
        AuctionSimulationConfig(n_simulations=1001)
...
def test_budget_residual_never_negative():
    result = simulate_auction(
        ...
        [
            BidderProfile(
                participant_id="u0",
                policy=BidderPolicy(aggressiveness=0.9, max_overpay_ratio=1.5),
            ),
            BidderProfile(
                participant_id="u1",
                policy=BidderPolicy(aggressiveness=0.9, max_overpay_ratio=1.5),
        ...
        _config(),
        _small_pool(),
        AuctionSimulationConfig(n_simulations=10, random_seed=7),
    )
...
def test_single_bidder_completes():
    result = simulate_auction(
        [
            ParticipantSetup(
                participant_id="solo", display_name="Solo", budget_initial=500
            )
        ],
        [BidderProfile(participant_id="solo", policy=BidderPolicy())],
        AuctionConfig(
            num_participants=1,
            role_quotas={"P": 1, "D": 2, "C": 2, "A": 1},
            budget_initial=500,
            reference_budget=300,
        ),
        _small_pool(),
        AuctionSimulationConfig(n_simulations=8, random_seed=1),
    )
...
def test_bidder_policy_validation():
    with pytest.raises(ValueError):
        BidderPolicy(aggressiveness=1.5)
    with pytest.raises(ValueError):
        BidderPolicy(max_overpay_ratio=0.5)
