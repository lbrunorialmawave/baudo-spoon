...
from ml.auction.models import AuctionConfig, AuctionState, ParticipantSetup, ParticipantState
...
class BidderPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if self.max_overpay_ratio < 1.0:
            raise ValueError(f"BidderPolicy.max_overpay_ratio must be >= 1.0, got {self.max_overpay_ratio}")
        ...
        if self.rebid_trigger_pct_above_expected < 0:
            raise ValueError("BidderPolicy.rebid_trigger_pct_above_expected must be >= 0")
    ...
    def to_summary_dict(self) -> dict[str, Any]:
        return {
            ...
            "player_acquisition_probability": {
                pid: {"prob": round(stats["prob"], 4), "avg_price": round(stats["avg_price"], 2)}
...
def _remaining_slots(participant: ParticipantState, role_quotas: Mapping[str, int]) -> dict[str, int]:
    return {
...
def _bot_bid(
    ...
    if not [r for r in roles if remaining.get(r, 0) > 0]:
        return None
    if policy.min_start_probability is not None and player.start_probability is not None:
        if float(player.start_probability) < policy.min_start_probability:
            return None
...
def _resolve_winner(bids: dict[str, float], min_step: int = 1) -> tuple[str, float] | None:
    if not bids:
        return None
...
def _all_squads_full(state: AuctionState) -> bool:
    return all(_squad_full(p, state.config.role_quotas) for p in state.participants.values())


def _run_one_scenario(
    participants, config, player_pool, profiles, scenario_idx, base_seed, price_noise_std_ratio, min_bid_step,
) -> AuctionState:
    ...
    for round_i in range(len(player_pool) + 5):
        ...
        for pid, pstate in state.participants.items():
            ...
            if bid is not None and bid >= 1:
                bids[pid] = bid
        if not bids:
            state.available_pool = [p for p in state.available_pool if p.player_id != player.player_id]
            continue
        ...
        result = record_assignment(
            state, player_id=player.player_id, winner_participant_id=winner_id, final_price=price_int,
        )
...
def simulate_auction(
    ...
    missing = [p.participant_id for p in participants if p.participant_id not in profiles]
    if missing:
        default = BidderPolicy()
        for pid in missing:
            profiles[pid] = default
        warnings.append(f"Default BidderPolicy applied to participants without profile: {missing}")
    ...
    for k in range(sim_config.n_simulations):
        if sim_config.timeout_seconds > 0 and (time.perf_counter() - t0) > sim_config.timeout_seconds:
            warnings.append(f"Simulation stopped early at {k}/{sim_config.n_simulations} due to timeout")
            break
        state = _run_one_scenario(
            participants, config, player_pool, profiles, k,
            sim_config.random_seed, sim_config.price_noise_std_ratio, sim_config.min_bid_step,
        )
        n_completed += 1
        price_index_samples.append(
            {str(role): {str(t): float(v) for t, v in tiers.items()} for role, tiers in state.price_index.items()}
        )
        for pid, pstate in state.participants.items():
            initial_budget = next(
            ...
            completed_flags[pid].append(1 if _squad_full(pstate, config.role_quotas) else 0)
    ...
    if price_index_samples:
        for role in price_index_samples[0]:
            price_index_drift_p50[role] = {
                tier: _percentile(
                    [s[role][tier] for s in price_index_samples if role in s and tier in s[role]], 50,
                )
    ...
    for player_id, events in acquisition.items():
        ...
        player_acq[player_id] = {"prob": len(events) / max(n_completed, 1), "avg_price": float(np.mean(prices))}
