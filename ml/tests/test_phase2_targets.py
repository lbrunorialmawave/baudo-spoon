...
from ml.domain.config import DEFAULT_ROLE_WEIGHTS
...
class TestTheoreticalFantavoto:
    ...
    def test_gk_scores_differently_than_fwd(self):
        ...
        assert result[0] != result[1], "GK and FWD with same stats should score differently"
...
class TestExpectedMinutesModel:
    ...
    def test_build_features_lag_correctness(self):
        ...
        assert p1.iloc[1]["mins_played_lag1"] == 2700.0, "Season 2023 lag should be 2022 value"
        assert p1.iloc[2]["mins_played_lag1"] == 1800.0, "Season 2024 lag should be 2023 value"
        assert pd.isna(p1.iloc[0]["mins_played_lag1"]), "Season 2022 has no prior, should be NaN"
