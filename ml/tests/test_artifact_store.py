...
def test_save_json_calls_r2_upload(store: ArtifactStore) -> None:
    ...
    args = store._s3.upload_file.call_args.args
    assert args[1] == "test-bucket"
    assert args[2] == "foo.json"


def test_save_json_upload_failure_is_non_fatal(
...
def test_save_json_no_tmp_file_left_behind_on_write_error(
...
def test_load_json_reads_local_without_touching_r2(
    store: ArtifactStore, tmp_path: Path
) -> None:
...
def test_load_json_falls_back_to_r2_when_local_missing(
    store: ArtifactStore, tmp_path: Path
) -> None:
    ...
    result = store.load_json("foo.json")
    assert result == {"b": 2}
    store._s3.download_file.assert_called_once_with(
        "test-bucket", "foo.json", str(tmp_path / "foo.json")
    )
...
def test_load_json_without_r2_config_returns_none_when_local_missing(
    tmp_path: Path,
) -> None:
    s = ArtifactStore(local_dir=tmp_path, r2_config=None)
...
def test_exists_remote_only_when_r2_populated_and_local_empty(
    store: ArtifactStore,
) -> None:
...
def test_r2_config_from_env_reads_prefixed_vars(
...
def test_r2_config_not_configured_without_endpoint() -> None:
    cfg = R2Config(
        endpoint_url=None, access_key_id=None, secret_access_key=None, bucket_name="b"
    )
    assert cfg.is_configured is False
...
def test_load_json_async_delegates_to_sync(
    store: ArtifactStore, tmp_path: Path
) -> None:
    (tmp_path / "foo.json").write_text('{"a": 1}', encoding="utf-8")
